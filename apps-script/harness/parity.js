...
for(const c of cases){
  ...
  // Some derivations resolve ids to names through the stored name map, so a case
  // can carry the map that was live when it was captured.
  if(c.names) ctx.PropertiesService.getScriptProperties().setProperty('NAMES', JSON.stringify(c.names));
  post(ctx,c.payload);
